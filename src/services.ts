/**
 * Data layer — all operations via Supabase.
...
function fromNote(n: Record<string, unknown>): Note {
  return {
    id: n.id as string,
...
export async function addClipboardItem(
  ...
  const user = getCurrentUser();
  if (!user) return;
  const { error } = await sb().from("clipboard_items").insert({
    user_id: user.id,
    content,
  ...
  if (error) throw error;
  // Prune: keep only 100 most recent items
...
/** Copy a base64 PNG image to the system clipboard via Tauri. */
export async function copyImageToClipboard(imageB64: string): Promise<void> {
  return invoke("copy_image_to_clipboard", { imageB64 });
}
