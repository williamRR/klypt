...
function initScrollAnimations() {
    ...
    if (heroClipsEl) {
      ...
      `).join('');
        ...
        `;
                firstClip.style.animation = 'none';
                firstClip.offsetHeight;
                firstClip.style.animation = 'slideIn 0.4s var(--ease-snap)';
...
function initDemoClips() {
    ...
    if (!generateBtn || !outputContentEl)
        return;
    clips.forEach(clip => {
    ...
    generateBtn.addEventListener('click', () => {
        ...
        if (selectedClips.length === 0) {
            if (outputContentEl)
                outputContentEl.innerHTML = 'Please select at least one clip to generate documentation.';
            return;
        }
